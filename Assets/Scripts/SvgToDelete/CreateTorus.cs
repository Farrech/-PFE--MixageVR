...
public class CreateTorus : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int seg = 0; seg <= nbRadSeg/2; seg++)
        {
            int currSeg = seg == nbRadSeg ? 0 : seg;

            float t1 = (float)currSeg / nbRadSeg * _2pi;
            Vector3 r1 = new Vector3(Mathf.Cos(t1) * minorRadius, 0f, Mathf.Sin(t1) * minorRadius);
            ...
            for (int side = 0; side <= nbSides; side++)
            {
                ...
                Vector3 r2 = Quaternion.AngleAxis(-t1 * Mathf.Rad2Deg, Vector3.up) * new Vector3(Mathf.Sin(t2) * majorRadius, Mathf.Cos(t2) * majorRadius);
